...
await testcaeadd(5,mange,questions[i]);
...
async function testcaeadd(i,manage,question){
  ...
  let input=await driver.findElement(swd.By.css(".formgroup.horizontal.input-testcase-row.row .CodeMirror.cm-s-default.CodeMirror-wrap"));
  await input.sendKeys(question["Testcases"][j]["Input"]);
   let input=await driver.findElement(swd.By.css(".formgroup.horizontal.output-testcase-row.row .CodeMirror.cm-s-default.CodeMirror-wrap"));
  await input.sendKeys(question["Testcases"][j]["Output"]);
